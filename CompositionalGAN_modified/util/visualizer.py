...
class Visualizer():
    ...
    def display_current_results(self, visuals, epoch, save_result, stepsize=1,im_name='',n_latest=50):
        ...
        if self.use_html and (save_result or not self.saved):  # save images to a html file
            ...
            for n in range(epoch, max(epoch-stepsize*n_latest,-1), -stepsize):
                webpage.add_header('step [%d] name [%s]' %(n,im_name))
                ims = []
                txts = []
                links = []

                for label, image_numpy in visuals.items():
                    img_path = 'step%.3d_%s_%s.png' % (n, label,im_name)
                    ims.append(img_path)
                    txts.append(label)
    ...
    # save image to the disk
    def make_html(self, webpage,visuals, image_path):
        image_dir = webpage.get_image_dir()
        short_path = ntpath.basename(image_path[0])
        name = os.path.splitext(short_path)[0]
